        ...
        for (const processed of this.queue.process(data)) {
            if (processed.id === null) {
                ...
                if (this.status === ServerSocketStatus.Connected) {
                    this.server.emit('error', MessageError_1.makeError('Failed to parse message', processed.data), this);
                }
                else {
                    this.server.emit('error', MessageError_1.makeError('Failed to process message during connection, calling disconnect', processed.data), this);
                    this.disconnect();
                }
            }
            else {
